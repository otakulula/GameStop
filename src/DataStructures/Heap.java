...
public class Heap<T> {
    ...
    public void buildHeap() {
        int start = (int) Math.floor(heapSize/(double) 2);
    ...
    public void insert(T key) {
        this.heapSize++;
        heap.add(heapSize, getElement(heapSize - 1)); /// TEMP SOLUTION
    ...
    public int getLeft(int index) throws IndexOutOfBoundsException {
        if(index <= 0 || index > this.heapSize){
            throw new IndexOutOfBoundsException("getLeft: the index is out of bounds");
        }
        return index * 2;
    }
    ...
    public int getRight(int index) throws IndexOutOfBoundsException {
        if(index <= 0 || index > this.heapSize){
            throw new IndexOutOfBoundsException("getRight: the index is out of bounds");
        }
        return (index * 2) + 1;
    }
    ...
    public int getParent(int index) throws IndexOutOfBoundsException {
        if(index <= 1 || index > this.heapSize){
            throw new IndexOutOfBoundsException("getParent: the index is out of bounds");
        }
        return (int) Math.floor(index /(double) 2);
    }
    ...
    public T getElement(int index) throws IndexOutOfBoundsException {
        if(index <= 0 || index > this.heapSize){
            throw new IndexOutOfBoundsException("getElement: the index is out of bounds");
        }
    ...
    public ArrayList<T> sort() {
        ...
        ArrayList<T> copyHeap = new ArrayList<>();

        for (int i = 0; i < heap.size(); i++) {
            copyHeap.add(i, heap.get(i));
        }
